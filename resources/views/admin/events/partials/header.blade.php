...
<div class="relative w-full h-24 md:h-72 bg-gradient-to-r {{ $coverGradient }} rounded-[24px] md:rounded-[32px] py-2.5 px-4 md:p-10 flex flex-col justify-end overflow-hidden shadow-sm border border-white/10"
    ...
    <div class="relative z-10 space-y-1 md:space-y-4">
        <div class="flex items-center gap-2 md:gap-3 flex-wrap">
            <span class="bg-white/20 backdrop-blur text-white px-2 py-0.5 md:px-3.5 md:py-1 rounded-full text-[9.5px] md:text-xs font-bold uppercase tracking-wider border border-white/20">
                {{ $event->category }}
            </span>
            <span class="{{ $statusColor }} bg-opacity-95 px-2 py-0.5 md:px-3.5 md:py-1 rounded-full text-[9.5px] md:text-xs font-extrabold uppercase tracking-wide">
                {{ $statusText }}
            </span>
        </div>

        <h2 class="text-white text-sm sm:text-3xl md:text-5xl font-black tracking-tight leading-tight drop-shadow-sm truncate max-w-4xl">
            {{ $event->name }}
        </h2>

        <div class="flex items-center gap-2 md:gap-6 text-white/90 text-[10px] md:text-sm font-semibold flex-wrap">
            <span class="flex items-center gap-1.5 md:gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3.5 h-3.5 md:w-4 md:h-4 text-white/70">
            ...
            @if($eventStatus === 'ongoing')
                <span class="flex items-center gap-1.5 md:gap-2 text-rose-350">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3.5 h-3.5 md:w-4 md:h-4 text-rose-450 flex-shrink-0 animate-pulse">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                    </svg>
                    <span class="font-extrabold uppercase tracking-wide text-[10px] md:text-xs">Ends In:</span>
                    <span id="countdownTimer" data-end-time="{{ $event->end_date }} {{ $event->end_time }}" class="font-black">Calculating...</span>
                </span>
            @else
                <span class="flex items-center gap-1.5 md:gap-2">
                    <svg xmlns="http://www.w3.org/2050/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3.5 h-3.5 md:w-4 md:h-4 text-white/70">
...
<!-- Tab Navigation Bar -->
<div class="bg-white rounded-2xl md:rounded-[32px] border border-gray-100 shadow-sm overflow-hidden p-1 md:p-2 mt-2 md:mt-6">
    <div class="overflow-x-auto whitespace-nowrap scrollbar-none w-full pb-1 md:pb-0">
        <div class="flex items-center gap-1">
        <button onclick="switchTab('overview')" id="tab-btn-overview" 
            class="tab-btn flex items-center gap-1.5 md:gap-2 px-3.5 py-1.5 md:px-6 md:py-3.5 rounded-lg md:rounded-2xl text-[11px] md:text-sm transition duration-200 cursor-pointer 
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.2" stroke="currentColor" class="w-3.5 h-3.5 md:w-4 md:h-4">

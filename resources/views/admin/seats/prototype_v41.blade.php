...
@section('title', 'Seat Configuration Builder V4.1')
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
                ...
                Seat Configuration Builder (V4.1 Seating Upgrade)
            </h2>
            <p class="text-gray-500 mt-1 text-sm">Define seating rules. The system dynamically generates linear theater rows, classroom table layouts, or curved concert arenas.</p>
        </div>
        ...
        <div id="sidebar-column" class="space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div id="seats-settings-group" class="space-y-4">
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="number" id="vip-seats" value="20" min="0" max="200" oninput="triggerGenerate()" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 font-bold transition duration-200">
                        </div>

                        <!-- Platinum Seats Count -->
                        <div class="space-y-1">
                            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Platinum Seats</label>
                            <input type="number" id="plat-seats" value="50" min="0" max="400" oninput="triggerGenerate()" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 font-bold transition duration-200">
                        </div>

                        <!-- Regular Seats Count -->
                        <div class="space-y-1">
                            <label class="text-xs font-bold text-gray-400 uppercase tracking-wider block">Regular Seats</label>
                            <input type="number" id="reg-seats" value="100" min="0" max="1000" oninput="triggerGenerate()" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 font-bold transition duration-200">
                        </div>
            ...
            <div id="stats-card" class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-4 text-center col-span-2">
                        <span class="text-[10px] text-slate-400 font-extrabold block uppercase tracking-wider">Total Seating Capacity</span>
                        <span id="stat-total" class="text-3xl font-black text-emerald-400 block mt-1">0</span>
                    </div>
                    <div class="bg-amber-50 border border-amber-100/30 rounded-2xl p-3 text-center">
                        <span class="text-[9px] text-amber-500 font-extrabold block uppercase tracking-wider">VIP</span>
                        <span id="stat-vip" class="text-lg font-black text-amber-600 block mt-0.5">0</span>
                    </div>
                    <div class="bg-purple-50 border border-purple-100/30 rounded-2xl p-3 text-center">
                        <span class="text-[9px] text-purple-500 font-extrabold block uppercase tracking-wider">Platinum</span>
                        <span id="stat-plat" class="text-lg font-black text-purple-600 block mt-0.5">0</span>
                    </div>
                    <div class="bg-blue-50 border border-blue-100/30 rounded-2xl p-3 text-center col-span-2" id="stat-reg-box">
                        <span class="text-[9px] text-blue-500 font-extrabold block uppercase tracking-wider">Regular</span>
                        <span id="stat-reg" class="text-lg font-black text-blue-600 block mt-0.5">0</span>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function loadPrototype() {
        const stored = localStorage.getItem('joyvent_seat_configuration_v41_prototype');
        if (stored) {
            try {
                ...
                if (loaded && loaded.config) {
                    ...
                    document.getElementById('reg-seats').value = loaded.config.regSeats !== undefined ? loaded.config.regSeats : 100;
                    document.getElementById('seats-per-row').value = loaded.config.seatsPerRow !== undefined ? loaded.config.seatsPerRow : 10;
                    document.getElementById('layout-aisle').checked = loaded.config.hasAisle !== undefined ? loaded.config.hasAisle : true;
                    document.getElementById('class-rows').value = loaded.config.classRows !== undefined ? loaded.config.classRows : 5;
                    document.getElementById('class-seats-per-row').value = loaded.config.classSeatsPerRow !== undefined ? loaded.config.classSeatsPerRow : 8;
                ...
                console.error('Failed to parse localStorage prototype V4.1, loading defaults', e);
    ...
    function generateDefaultLayout() {
        ...
        document.getElementById('reg-seats').value = 100;
        document.getElementById('seats-per-row').value = 10;
    ...
    function triggerGenerate() {
        ...
        generateLayout(layoutType, vipSeats, platSeats, regSeats, hasAisle, classRows, classSeatsPerRow);
        savePrototype();
        renderCanvas();
        updateRealtimeStats();
        updateJsonPreview();
    ...
    function triggerGenerateButton() {
        triggerGenerate();
        alert('Seating layout generated successfully!');
    }

    // Core layout generator algorithms
    function generateLayout(layoutType, vipCount, platCount, regCount, hasAisle, classRows, classSeatsPerRow) {
        seats = [];
        desks = [];
        ...
        if (layoutType === 'theater') {
            ...
            const seatsPerRow = parseInt(document.getElementById('seats-per-row').value, 10) || 10;
            const rowSpacing = 50;
            const categoryGap = 55; // Tighter category gap
            const centerY = -2400; // Focal point pushed further back for gentler curve (rotation capped at 5-8 degrees)
            const centerX = canvasWidth / 2; // 365
            const concertAisleGap = hasAisle ? 20 : 0; // Tighter center gap (20px)

            const categories = [
                { type: 'VIP', count: vipCount, prefix: 'VIP', widthScale: 1.00 },
                { type: 'Platinum', count: platCount, prefix: 'PLAT', widthScale: 1.20 },
                { type: 'Regular', count: regCount, prefix: 'REG', widthScale: 1.45 }
            ];

            // Scan active categories and rows to calculate the maximum row multiplier
            let maxRowMultiplier = 1;
            categories.forEach(cat => {
                if (cat.count > 0) {
                    const totalRows = Math.ceil(cat.count / seatsPerRow);
                    const r = totalRows - 1; // last row of this category
            ...
            // Base seat spacing scaled to fit the max expanded row width within the 730px canvas safely (target max row width 670px)
            let baseSeatSpacing = (670 - concertAisleGap) / ((seatsPerRow - 1) * maxRowMultiplier);
            baseSeatSpacing = Math.min(60, Math.max(34, baseSeatSpacing)); // Safe bounds: min 34px to prevent seat overlap, max 60px

            let currentY = 45; // Start closer to stage
            let currentGlobalRowIndex = 0;
            ...
            categories.forEach((cat, catIdx) => {
                ...
                // Spacing for this category based on width scale
                const catSpacing = baseSeatSpacing * cat.widthScale;

                for (let r = 0; r < totalRows; r++) {
                    const rowLabel = getRowLabel(currentGlobalRowIndex);
                    const seatsInRow = Math.min(seatsPerRow, cat.count - seatsPlaced);
                    
                    // Row-by-row fan expansion within the category (4% per row)
                    const seatSpacing = catSpacing * (1 + r * 0.04);

                    // Row width calculation (horizontal width before curving)
                    ...
                    for (let i = 0; i < seatsInRow; i++) {
                        // Horizontal position relative to center of the row
                        ...
                        // Center Compression: compress horizontal coordinates dx slightly near the center
                        let compressedDx = dx;
                        if (rowWidth > 0) {
                            compressedDx = dx * (0.88 + 0.12 * Math.abs(dx / (rowWidth / 2)));
                        }

                        // Map horizontal coordinate dx to polar angle theta on the curve
                        const theta = compressedDx / radius;

                        // Calculate curved coordinates
                    ...
                    // If it is the last row of the current category, and there are more categories, add category gap
            ...
            // Calculate total width of a row to center it on the canvas
            let totalWidth = 0;
            ...
            for (let r = 0; r < classRows; r++) {
                ...
                // Categorize rows: first row VIP, second row Platinum, rest Regular
                const catType = (r === 0) ? 'VIP' : ((r === 1) ? 'Platinum' : 'Regular');
        ...
        // Horizontal post-processing centering for all layout types
        if (seats.length > 0) {
            let minX = Infinity;
            let maxX = -Infinity;
    ...
    function savePrototype() {
        ...
        localStorage.setItem('joyvent_seat_configuration_v41_prototype', JSON.stringify(payload));
    }
    ...
    function copyJsonOutput() {
        ...
        navigator.clipboard.writeText(json).then(() => {
            alert('V4.1 JSON output copied to clipboard successfully!');
        }).catch(err => {
